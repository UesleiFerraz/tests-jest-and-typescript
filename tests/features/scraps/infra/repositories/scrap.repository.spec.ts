...
describe("Scrap repository", () => {
  ...
  describe("Get all", () => {
    ...
    it("Should pass an userUid as a parameter when call this function", async () => {
      ...
      spy.mockResolvedValue([await makeScrap()]);
  ...
  describe("Delete", () => {
    ...
    it("Should delete an scrap if all params are valid", async () => {
      const sut = new ScrapRepository();
      const scrap = await makeScrap();
      const result = (await sut.delete(scrap.uid, scrap.userUid)) as any;
